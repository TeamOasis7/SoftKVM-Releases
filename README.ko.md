...
4. **정지**를 누르면 연결이 끊기고 초기 화면으로 돌아갑니다. 제어 PC에서 `K`와 `M`을 1초간 함께 누른 뒤 `E`를 누르면 입력과 관련 프로세스를 정리하고 Soft KVM을 자동으로 다시 실행합니다.

제어 코드 입력 화면에서 입력칸 오른쪽의 `×`를 누르면 연결을 시작하지 않고 초기 화면으로 돌아갑니다.

각 PC에서 선택한 사용 모니터 좌표를 서로 비례 변환합니다. 연결 중 **일시정지**를 누르거나 `K+M` 명령창에서 `Z`를 누르면 연결을 유지한 채 입력 전달만 멈출 수 있습니다. 연결되면 양쪽 창은 선택 모니터 우측 상단으로 이동합니다.

톱니바퀴 버튼을 누르면 다음 설정이 아래로 펼쳐집니다.

- 윈도우 시작 시 자동 시작
- 대기 모드 시 최소화
- 제어 클릭 차단 (`K+M` 명령창의 `X`로 켜기/끄기)
- 제어 키보드 차단 (`K+M` 명령창의 `K`로 켜기/끄기)
- 키보드 제어
- 마우스 제어
- 게임 입력 모드 (`K+M` 명령창의 `G`로 켜기/끄기)
- 연결 그룹 이름
- 스왑모드 및 스왑지점 (`좌측`, `우측`, `상단`)

설정값은 현재 Windows 사용자 계정에 저장됩니다. `키보드 제어`는 대기 PC로 보내는 원격 키보드 기능 자체를 켜고 끕니다. `제어 키보드 차단`은 키보드를 대기 PC에 계속 전달하면서 제어 PC의 로컬 키 입력만 임시로 막습니다. `제어 클릭 차단`도 원격 클릭은 전달하면서 제어 PC의 로컬 클릭을 막되, Soft KVM 창 내부 클릭은 허용합니다. `K`와 `M`을 1초간 함께 누르면 제어 PC에 명령 목록이 열리며, `X`, `K`, `G`, `Z`, `E` 중 하나를 눌러 기능을 실행하거나 `Esc`로 닫을 수 있습니다. 평상시 `Alt` 조합과 `Esc`는 원격 PC로 그대로 전달됩니다.

`게임 입력 모드`는 키보드 스캔 코드와 상대 마우스 이동을 사용합니다. 제어 PC의 커서를 주 모니터 중앙에 재고정해 화면 끝에서도 게임 시점을 계속 움직일 수 있으며 `K+M` 명령창의 `G`로 즉시 해제합니다. 일반 데스크톱과 게임 메뉴의 절대 좌표 클릭에는 게임 입력 모드를 끄고 사용합니다. Windows 합성 입력을 차단하는 안티치트 게임은 지원하지 않습니다.
